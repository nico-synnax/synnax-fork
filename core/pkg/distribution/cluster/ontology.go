...
// OntologyID returns a unique identifier for a Cluster to use with a resource Ontology.
...
// NodeOntologyService implements the ontology.Service interface to provide resource
// access to a cluster's nodes.
...
func (s *NodeOntologyService) ListenForChanges(ctx context.Context) {
	if err := s.
		Ontology.
		NewWriter(nil).
		DefineResource(ctx, NodeOntologyID(Free)); err != nil {
...
// OntologyService implements the ontology.Service to provide resource access to
