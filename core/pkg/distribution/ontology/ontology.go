...
// schema.
func NewResource(schema zyn.Schema, id ID, name string, data any) resource.Resource {
...
func Open(ctx context.Context, configs ...Config) (*Ontology, error) {
	...
	if err = o.
		NewRetrieve().
		WhereIDs(RootID).
		Exec(ctx, cfg.DB); errors.Is(err, query.NotFound) {
...
type Writer interface {
	...
	DeleteOutgoingRelationshipsOfType(
		ctx context.Context,
		from ID,
		t RelationshipType,
	) error
	...
	DeleteIncomingRelationshipsOfType(
		ctx context.Context,
		to ID,
...
func (o *Ontology) Search(ctx context.Context, req search.Request) ([]Resource, error) {
	...
	if err = o.
		...
		WhereIDs(ids...).
		Entries(&resources).
		Exec(ctx, o.DB); err != nil && !errors.Is(err, query.NotFound) {
		return nil, err
	}
